...
test('standalone batch workbench searches source email before preview', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /批量投递工作台/u }).first()).toBeVisible()
...
test('job body and batch delivery use separate interfaces', async ({ page }) => {
  ...
  await page.locator('.experience-command-actions').getByRole('button', { name: '批量投递', exact: true }).click()
  ...
  await mainNav.getByRole('button', { name: '历史', exact: true }).click()
  const draftGuard = page.getByRole('alertdialog')
  await expect(draftGuard).toBeVisible()
  await draftGuard.getByRole('button', { name: '放弃修改' }).click()
  await expect(page.getByRole('heading', { name: '历史任务', exact: true }).first()).toBeVisible()
  await expect(page.getByRole('region', { name: '批量投递工作台' })).toHaveCount(0)
  await mainNav.getByRole('button', { name: '任务', exact: true }).click()
  await page.locator('.experience-command-actions').getByRole('button', { name: '批量投递', exact: true }).click()
  await expect(page).toHaveURL(/\/batch\?screen=workspace$/u)
  await expect(page.getByRole('region', { name: '批量投递工作台' })).toBeVisible()
})
