...
- **正文与卡片同步落盘**：仅将已采集完整正文的记录发布为岗位卡或内容卡；正文待采记录保留在原任务检查点中，由“一键智能补全”继续采集后再进入 AI 解析。
- **补全与 AI 恢复**：补全流程复用原任务和正文账本，页面关闭或 Relay 页面失效时自动重建采集页；用户已选择模型时优先恢复该模型，未配置模型时使用可用的本地默认能力。
- **解析与质量状态**：按职责、要求等正文分区提取并保留证据，区分待质量检查、质量失效、质量未通过和 AI 失败初稿；AI 调用统一受总时限约束，超时会清理完整子进程树。
- **受众头像修复**：补充头像来源校验、历史污染识别和仅修复头像的恢复路径，优先从已采评论、帖子和公开主页补回可信头像。
- **SMTP 本机持久化**：SMTP 凭据使用本机独立密钥和 AES-256-GCM 加密保存在 Git 忽略目录，服务重启后可恢复使用；配置查询接口和任务历史不返回明文密码。
- **回归验证**：新增正文补全、AI 超时、受众头像、任务恢复和 SMTP 跨进程重启持久化测试。

### 投递中心附件与真人化文案

- **原任务内附件工作区**：投递文案编辑器现支持上传、候选人资料、任务 Artifact 与 Cover Letter 附件，并在当前 Job 工作目录持久化校验结果；发送前可查看完整正文、收发件人、草稿版本与实际附件清单。
- **可审计的真实发送**：附件会按 ID 在服务端重新校验并交给现有 Nodemailer 链路；每次发送生成不可变 Send Bundle，幂等身份包含草稿、收件人和附件集合，审计只记录文件名、类型、大小与 SHA-256，不写入附件正文或本地绝对路径。
- **证据约束文案**：投递邮件按岗位证据、候选人证据和投递上下文生成，默认采用短段落与事实表达；质量检查新增事实依据、具体性、自然度、重复度、附件一致性和 AI 套话等十个维度。
